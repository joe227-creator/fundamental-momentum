...
def run_optuna(config_path, params_path, reference_path, artifact_dir, optuna_config_path, perturb_runs, perturb_sigma, seed):
    ...
    study = optuna.create_study(
        ...
        storage=f"sqlite:///{root / 'optuna_multi_lookback_momentum_blend.db'}",
    ...
    def objective(trial):
        value = trial.suggest_float("momentum_blend", 0.0, 1.0)
        metrics, _ = _evaluate(config_path, base, {"momentum_blend": value}, ref)
    ...
    bm, prepared = _evaluate(config_path, base, best, ref)
    seeds = []
