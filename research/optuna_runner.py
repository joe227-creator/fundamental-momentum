...
def run_optuna(config_path, params_path, reference_path, artifact_dir,
    ...
    study = optuna.create_study(
        ...
        storage=f"sqlite:///{root / 'optuna_robust_median_rank_consensus.db'}",
    ...
    for i in range(max(0, int(perturb_runs))):
        metrics, _, _, _ = _evaluate_result(
            prepared, _select(prepared, seed=seed + 1000 + i, sigma=perturb_sigma), baseline_turnover
        )
    ...
    for key in ("research_score", "mean_rolling_6m_return", "return_on_risk", "win_rate",
                "maximum_drawdown", "sharpe", "turnover", "perturb_score_mean",
                "perturb_score_std", "perturb_score_min", "perturb_score_max"):
