"""Validate one frozen candidate without tuning its parameters."""
...
def run_validation(config_path: str, params_path: str, reference_path: str,
    ...
    sigma = float(validation.get("perturb_sigma", 0.03))
    seed_start = int(validation.get("seed_start", 1042))
    for index in range(int(validation.get("n_seeds", 10))):
        seed = seed_start + index
        metrics, _, _, _ = _evaluate_result(
            prepared, _select(prepared, seed=seed, sigma=sigma), baseline_turnover
        )
        metrics["seed"] = seed
    ...
    for name, start, end in (
        ...
        subset = windows[
            (windows["start_date"] >= start) & (windows["end_date"] <= end)
        ]
        regime_metrics[name] = {
    ...
    metrics.update({
        "validation_candidate_score": base_metrics["research_score"],
        ...
        "validation_baseline_score": 0.6149878551980299,
        "validation_score_mean_beats_baseline": int(scores.mean() > 0.6149878551980299),
        "validation_min_beats_baseline": int(scores.min() > 0.6149878551980299),
        "validation_regimes": regime_metrics,
    })
    root = Path(artifact_dir)
    _write_artifacts(root, metrics, curve, windows, trades, seed_results, prepared["cfg"])
    (root / "regime_metrics.json").write_text(json.dumps(regime_metrics, indent=2, default=str), encoding="utf-8")
    for key in (
        "research_score", "validation_seed_score_mean", "validation_seed_score_median",
