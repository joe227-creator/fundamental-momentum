"""Validate fixed median rank consensus candidate."""
...
def run_validation(config_path: str, params_path: str, reference_path: str,
    ...
    for index in range(int(validation.get("n_seeds", 10))):
        ...
        metrics, _, _, _ = _evaluate_result(prepared, _select(prepared, seed=seed, sigma=float(validation.get("perturb_sigma", 0.03))), baseline_turnover)
        metrics["seed"] = seed
    ...
    for key in ("research_score", "validation_seed_score_mean", "validation_seed_score_median", "validation_seed_score_std", "validation_seed_score_min", "validation_seed_score_max", "validation_seed_return_std_over_mean", "validation_baseline_score", "validation_score_mean_beats_baseline", "validation_min_beats_baseline"):
