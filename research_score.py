...
def _load_forecast_uncertainty(
    ...
    if panel is None:
        raise RuntimeError("Missing quantile volume forecast cache")
    low = panel[[f"q2_h{h}" for h in range(horizon)]].mean(axis=1)
    mid = panel[[f"q5_h{h}" for h in range(horizon)]].mean(axis=1)
    high = panel[[f"q8_h{h}" for h in range(horizon)]].mean(axis=1)
